...
(function () {
  ...
  const ICON = {
    ...
    slash: svg('<circle cx="12" cy="12" r="9"/><path d="m5.5 5.5 13 13"/>'),
  ...
  function renderProfileSlot() {
    ...
    if (!isLoggedIn()) {
      slot.innerHTML = '<button class="btn btn-icon btn-ghost" id="profile-btn" title="sign in" aria-label="sign in">' + ICON.user + '</button>';
      $('#profile-btn').addEventListener('click', () => openAuthModal('login'));
      return;
    }
    ...
    slot.innerHTML =
      ...
      '<div class="menu-head"><b>' + escapeHtml(displayNameOf(user)) + '</b><span class="faint">@' + escapeHtml(user.login) + '</span></div>' +
      '<div class="sep"></div>' +
      '<a href="#/browse">' + ICON.search + ' browse</a>' +
      '<a href="#/following">' + ICON.user + ' following</a>' +
  ...
  function openAuthModal(tab = 'login') {
    ...
    root.innerHTML =
      ...
      '<h2>collab.dev</h2><p class="sub">sign in or create an account. GitHub is required to join.</p>' +
  ...
  async function render() {
    ...
    try {
      ...
      if (resource === 'following') return await viewFollowing(root);
  ...
  async function viewHome(root, query) {
    root.innerHTML =
      ...
      '<p class="home-sub">find developers to build with</p>' +
      ...
      '</form>' +
      '<p class="home-hint">' +
      (isLoggedIn() ? 'or <a href="#/browse">browse everyone</a> with filters' : 'sign in to browse developers') +
      '</p>' +
      '</div></div>';
  ...
  async function viewFollowing(root) {
    if (!requireGate(root)) return;
    root.innerHTML = '<div class="container"><div class="spread"><h1 class="page-title">following</h1>' +
      '<a class="btn btn-sm" href="#/browse">' + ICON.search + ' find more</a></div>' +
      '<div id="following-list" class="section"></div></div>';
    ...
    try {
      ...
      if (!contacts.length) {
        list.innerHTML = emptyState(ICON.user, 'not following anyone yet',
          'browse developers and follow the ones you want to build with.',
          '<a class="btn btn-primary" href="#/browse">browse developers</a>');
        return;
      }
      list.innerHTML = '<div class="stack">' + contacts.map((c) =>
        '<div class="contact-row">' + avatarHtml(c, 'sm') +
        '<div class="contact-main"><a class="contact-name" href="' + (c.userid ? '#/dev/' + c.userid : '#') + '">' +
        escapeHtml(c.displayName || c.login || 'Contact') + '</a>' +
        '<div class="contact-sub">' + (c.login ? '@' + escapeHtml(c.login) : escapeHtml(c.description || '')) +
        (c.jobTitle ? ' · ' + escapeHtml(c.jobTitle) : '') + '</div></div>' +
        '<div class="contact-actions">' +
        (c.userid ? '<button class="btn btn-sm btn-ghost" data-message="' + c.userid + '">' + ICON.mail + '</button>' : '') +
        '<button class="btn btn-sm" data-unfollow="' + c.userid + '" data-contact="' + c.contactid + '">unfollow</button>' +
        '</div></div>').join('') + '</div>';
  ...
  async function viewProfile(root, userid) {
    ...
    root.innerHTML =
      ...
      (isOwner
        ? '<a class="btn btn-sm" href="#/settings">' + ICON.settings + ' edit</a>'
